...
namespace OneNoteApplication.NoteService
{
    public class NoteHelper: INoteHelper
    {
        ...
        {
            string localPath = ConfigurationManager.AppSettings["localPath"] + "\\" + fileName;
            string recyclePath = localPath + "OneNote_RecycleBin\\" +  fileName;
            if (!Directory.Exists(recyclePath))
            {
                Directory.CreateDirectory(recyclePath);
            }
            if (localPath != recyclePath)
            {
                File.Move(localPath, recyclePath);
            }
        }
